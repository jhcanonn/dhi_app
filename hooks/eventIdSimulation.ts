const numerosGenerados: number[] = [] // Almacena los números generados

export function eventIdSimulation(min: number, max: number): number {
  const numero = Math.floor(Math.random() * (max - min + 1)) + min // Genera un número aleatorio entre min y max
