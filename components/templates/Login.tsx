...
import { DHI_SESSION, GET_TOKEN, PAGE_PATH, daysToMilliseconds } from '@utils';
...
const Login = ({ cookies }: { cookies: Cookies }) => {
  ...
  const setSessionCookie = (auth: AuthLogin) => {
    const expireDays = Number(process.env.NEXT_PUBLIC_TOKEN_EXPIRE_DAYS);
    const expires = new Date(
      new Date().getTime() + daysToMilliseconds(expireDays)
    );
  ...
  useEffect(() => {
    if (!cookies?.get(DHI_SESSION) && data) {
      setSessionCookie(data.auth_login);
