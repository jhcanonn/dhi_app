...
const FoliculosCapilarTable = ({ handleForm, disabledData }: Props) => {
  ...
  useEffect(() => {
    const suma = Object.values(RowsCodeFC).reduce(
      (accumulator, code) =>
        accumulator + (getValues(`foliculos_capilar_${code}_cantidad`) || 0),
      0,
    )
      ...
      <InputNumberValid
        ...
        onCustomChange={(e) => {
          ...
          Object.values(RowsCodeFC).forEach((code) => {
            const resta =
              currentValue - (getValues(`foliculos_capilar_${code}_de`) || 0)
            setValue(`foliculos_capilar_${code}_dr`, resta >= 0 ? resta : 0)
          })
          Object.values(RowsCodeFC).forEach((code) =>
            calcNoFoliculos(code, handleForm),
          )
        }}
      />
